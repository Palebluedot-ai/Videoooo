...
self_contained: true
required_context:
  - domain: "AGENTS.md §1 笔记真实性红线 / 代码即证据 / 密钥红线；docs/requirements.md 待拍板区 Q1；docs/notes/00-landscape.md（已验证事实）+ 01-learning-path.md（阶段划分与验证标准）"
...
- [x] ~~**工具版图笔记**~~：`docs/notes/00-landscape.md` 已于 2026-08-19 由 deep-research 产出并入仓（14 条确认结论带链接与日期，11 条被否单列存档）
- [ ] **选型理由落盘**：Q1 拍板后，把「为什么选这条」一句话写进 `docs/requirements.md` 已确认区
- [ ] **抽卡方差实感**（调研后新增，对应 01-learning-path S0.1）：同一条 prompt 重复生成 5 次，写清「哪些元素稳定、哪些每次都变」——没有这条实感，后面的自动化都是空中楼阁
...
- **一个 clip 只给一个运镜**（调研实证）：Veo 3.1 会静默降权运镜指令，叠加 dolly + pan + tilt 不稳定，遇矛盾指令倾向选最小运动
- **单价禁编**（D4 纪律）：笔记里不许出现未经当日官方定价页核实的 API 单价换算——本轮 4 条价格声称被对抗验证否掉 3 条
...
| 2026-08-19 | 1 | deep-research 摸清版图 | 105 agent / 验 25 条 | 确认 14 · 否 11；验收项 2 已达成 |
