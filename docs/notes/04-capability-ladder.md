...
> 本文是**你要的那份「一点一点学」的文档**的正文——**「学什么、怎么算过关」**。
> 配套的 **「具体怎么做」（工业级 SOP：8 阶段流水线 / 5-Aspect 分镜 / 三视图设定集）** 在 **[05-industrial-sop.md](05-industrial-sop.md)**。
> **「我应该怎么学」（读序 / 学习循环 / 反推法 / 学习台账）** 在 **[06-how-to-study.md](06-how-to-study.md)** —— **先读那份**。
...
> 📌 **2026-08-20 结构修正**：这 12 条原本是 L0（「拉片找破绽」），现已**降级为贯穿全程的验收检查表**——
> 工业界的 QC gate 本来就贯穿全程，不是入门考试。**L1 起每一级交付时都过一遍这张表。**
...
### L0 · 前期设定 —— 做设定集与分镜表
**这一级是方法，不是测试。** 工业流水线里 scene_plan 之前的所有活都在这儿：设定集、分镜、beat map。
完整 SOP 见 **[05-industrial-sop.md](05-industrial-sop.md)**。

| | |
|---|---|
| **判据** | 交付两份东西：① 一套角色 **turnaround**（正面 / 3-4 前 / 侧面 / 3-4 后 / 背面）+ 色标（每块标 HEX）② 一张 **5-Aspect 分镜表**，每个镜头五维全填 |
| **练习** | 给你想做的题材做完整前期包：beat map（Hook → Escalation → Reveal → Landing）→ turnaround → 5-Aspect 分镜表。**一帧视频都不生成** |
| **工具** | Nano Banana / Seedream 出 turnaround（prompt 见 05 号笔记第四节）· 表格软件写分镜 |
| **典型卡点** | 想跳过直接写 prompt。**五段式是表达层，5-Aspect 是规划层**——没想清楚，翻译得再漂亮也是空的 |
...
| | |
|---|---|
| **判据** | 一条 5–10 秒片段，**12 破绽验收表 0 命中**（这张表从这一级起每级都过） |
...
### L3 · 一致性工程
> **喂参考图，喂的应该是 L0 做的 turnaround，不是随手一张图。** 一张正面照只锁住正面，模型转侧面就自由发挥了；turnaround 锁 360°。这是「角色一致性」从玄学变成工程的地方。⚠️ 这条是**我的推论**，没找到直接实证——是你这一级该亲手验的第一件事。
